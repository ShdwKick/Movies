...
module.exports = function createPoiskKino(options = {}) {
  ...
  /** Единственное место, откуда уходят HTTP-запросы к provider'у.
      ...
      могли быть совершенно свободны. Теперь 403 ИЛИ 401 от provider'а —
      сигнал «этот ключ сейчас бесполезен (квота/битый токен), независимо от
      того, что говорит наш счётчик»: помечаем его достигшим dailyCap и
      пробуем следующий ключ по кругу (pickKey уже пропустит помеченный), а
      не молча повторяем на нём же или сразу сдаёмся. Обычный `!res.ok` для
      остальных кодов (404/400/5xx) — как раньше, сразу наружу, это не про
      квоту/ключ. */
  async function call(pathAndQuery) {
    ...
    while (idx !== -1) {
      ...
      if (res.status === 403 || res.status === 401) {
        // 403 — «квота на сегодня кончилась» (см. коммент выше). 401 —
        // ключ невалиден/отозван (опечатка при копировании, просроченный
        // токен): без ротации ОДИН битый ключ на первом месте в списке
        // валил бы вообще ВСЁ, включая рабочие ключи после него — молча
        // помечаем и его «исчерпанным на сегодня» и едем дальше по кругу;
        // не идеально (сброс в полночь даст ему ещё один шанс, хотя он
        // просто сломан), но лучше полного отказа с непонятной причиной.
