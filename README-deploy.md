...
добавление НОВЫХ (ещё не закэшированных) фильмов будет отвечать 503. Положите
рядом с `docker-compose.prod.yml` файл `.env` с правами 600 (он читается
подстановкой `${POISKKINO_API_KEY:-}` в самом compose-файле):
